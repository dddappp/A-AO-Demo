...
function article_create_logic.verify(article_id, title, body, tags, cmd, msg, env)
    return article.new_article_created(article_id, title, body, tags, msg.Owner)
end
...
function article_create_logic.new(event, msg, env)
    return article.new(event.title, event.body, event.author, event.tags)
end
