...
在传统的 Web2 开发环境中，我们通常可以采用“事务性发件箱模式”[^TransactionalOutbox]来解决这一问题。
...
### 简化测试方式：两进程测试（Alice 和 Bob）

如果你还没有准备好进行完整的多进程部署，或者想要快速验证基本的 Saga 功能，可以使用这种简化的两进程测试方法。这种方法使用单进程模式生成的代码，但通过手动配置实现跨进程通信，适合快速原型验证和学习目的。
