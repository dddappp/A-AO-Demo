...
#### 6.1.3 **重要警告：消息属性不支持Boolean值**
- **严重限制**: AO消息系统**完全不支持**boolean值作为消息属性！
- **技术原因**: AO的消息序列化/反序列化机制无法处理boolean类型
- **表现症状**:
  - 消息看似发送成功（handler执行完成）
  - 但消息无法正确路由到目标Inbox
  - Inbox验证会超时失败
- **强制要求**: **所有消息属性必须使用字符串值**
- **正确做法**: `Transferable = tostring(booleanValue)` 而不是 `Transferable = booleanValue`
- **实际案例**: NFT转让状态设置功能因使用boolean值而消息丢失
- **开发者注意**: 这不是规范问题，而是AO平台的底层技术限制！
...
**核心问题：** 用户发送 `TokenId = "1"`，但在目标进程中需要用 `Tokenid`（首字母大写）访问。

**调试输出验证：**
```lua
=== GET-NFT DEBUG ===
msg.TokenId: nil          -- 大写版本不存在
msg.Tokenid: 1           -- 首字母大写版本存在
msg.Tags.TokenId: nil    -- 大写 Tags 不存在
msg.Tags.Tokenid: 1      -- 首字母大写 Tags 存在
```
